      ...
      class AnalyticsController < ApplicationController
        ...
        def summary
          render json: {
            ...
            top_paths: PageView.group(:path).order(count_all: :desc).limit(8).count.map do |k, v|
              { path: k, count: v }
            end,
            top_referrers: PageView.group(:referrer).order(count_all: :desc).limit(8).count.map do |k, v|
              { referrer: k, count: v }
            end
          }
        end
